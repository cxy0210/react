...
class ShopCar extends React.Component {
	...
	calcAll = (ev) => {
		...
		for (var i = 0; i < cart.length; i++) {
			if (cart[i].is_select) {
				totalPrice += cart[i].price * cart[i].cart_number;
	...
	goto = () => {		
		if (this.state.num != 0) {
			alert('成功支付'+this.state.totalPrice+'元')
		...
		return (
			<div className="ShopCar">
				<header className="aui-header-default aui-header-fixed aui-header-bg">
					...
					<div className="aui-header-center aui-header-center-clear">
						<div className="aui-header-center-logo">
							<div className="aui-car-white-Typeface"  style={{ marginLeft: 5}}>购物车({car.length})</div>
						</div>
					</div>
							...
							<div className="aui-car-coupons">
								{
									car.length > 0 ?
										<div style={{ display: "flex" }}>
											...
											<Button type="warning" size={"small"} style={{ marginTop: 0, marginLeft: 20 }} onClick={this.removeAll}>全部删除</Button>
										</div> :
										<div style={{ display: "flex" }}>
											<i>购物车没有商品!!! </i>
									...
									car.map((item, index) => (
										...
										className="del"
											...
											right={[
												...
												// 	style: { backgroundColor: '#399', color: 'white', padding: '0 5px' },
												// },
												{
													text: ' 删 除 ',
													
													onPress: () => { this.remove(index) },
													style: { backgroundColor: '#F4333C', color: 'white', padding: '0 5px' },
												},
											]}
												...
												<li key={item._id}>
													<div className="aui-car-box-list-item">
														...
														<div className="aui-car-box-list-img" >
															<a href="#">
																<img src={item.banner} style={{ width: 80, height: 80, }} alt="" />
															</a>
														</div>
														<div className="aui-car-box-list-text" >
															<h4>
																<a href="#" style={{marginLeft:5,}}>{item.title}</a>
																{/* <a href="#">你好吗</a> */}
															</h4>
															<div className="aui-car-box-list-text-brief" >
																<span style={{marginLeft:5}}>{item.des}</span>
																{/* <span>{item.cat_three_id}</span> */}
																{/* <span>分类:4123</span>
																<span>重量:1212</span> */}
															</div>
															<div className="aui-car-box-list-text-price" >
																<div className="aui-car-box-list-text-pri" style={{marginLeft:5,}}>
				...
				<div className="aui-payment-bar">
					...
					<a className="settlement" onClick={() => { this.goto() }} style={{fontSize:16,}}>结算({this.state.num})</a>
				</div>
